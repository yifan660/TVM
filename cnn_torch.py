# in python/tvm/relay/frontend/cnn_torch.py
...
def _add_output_quant_params_to_scalar_op(node, graph, input_scale, input_zero_point, scalar):
    ...
    elif operator == "quantized::clamp":
        clamp = _get_clamp_output_quant_param(
            input_min, input_max
        )
...
def get_alpha(self, tensor, tag="", stat_id=None, clip_type='laplace', per_channel=False):
    if clip_type=='laplace':
        self.get_alpha_laplace(tensor, stat_id, per_channel=per_channel)
    elif clip_type=='gaus':
        self.get_alpha_gaus(tensor, stat_id, per_channel=per_channel)
    elif clip_type=='std':
        self.get_alpha_pstd(tensor, stat_id, per_channel=per_channel)
    elif clip_type=='mix':
