...
void get_omega(tensor sigma, int target_bins)    {
    float B;
    B = len(sigma) * target_bins
    ...
    tensor omega;
    for(int i=0;i<shape[0]*shape[1];i++)   {
        omega[i] = p[i]*B/std::accumulate(p.start, p.end);
    }
    return omega;
...
double get_alpha_mult(std::vector<double> omega, bool sym) {
    // omega is numpy vector
    // standard deviation is tensor 
    double omega = 
    if(sym==false)  {
        for(auto i:omega)   {
            i*=2;
...
def mid_tread_quantization(self, tensor, id, target, clip=False, sym=True):
    // get the standard deviation of entire tensor
...
float calculate_std(std::vector<float>& t) {
    // calculate variance
    // standard deviation is sqrt of variance
    double mean = std::accumulate(t.begin(),t.end());
    double var;
...
double mid_tread_quantization(std::vector<float> tensor, float target, bool clip, bool sym) {
    std::vector<float> std;     // standard deviation

    // get the standard deviation of each element of tensor
    for(int i=0;i<tensor.size();i+=len)   {
        // calculate standard deviation along dimension = 1
        vector<float>::const_iterator first = tensor.begin();
        vector<float>::const_iterator last = tensor.begin()+len;
        vector<float> temp(first, last);
        std.push_back() = calculate_std(temp);
    }
    
    if(clip==true)    {
        std::vector<float> alpha_mult = new_tensor();
        alpha_mult = tensor.new_tensor(get_alpha_mult(omega, sym=sym))
        mu = tensor.mean(dim=-1)
        b = torch.mean(torch.abs(tensor - mu.unsqueeze(-1)), dim=-1)
        if(sym==true)    {
            rng = (2*alpha_mult*b);
    ...
    if(clip)    {
        ...
        if(sym) {
            ...
            c_max = mu_q + omega;
            c_min = tensor.new_tensor[0];
        }

        Expr out = torch.min(out, c_max.unsqueeze(-1))
        Expr out = torch.max(out, c_min.unsqueeze(-1))
